...
<%@ tag import="com.flexink.common.utils.CommonCodeUtils" %>
<%@ tag import="com.flexink.domain.code.CommonCode" %>
...
<%
    ...
    switch (type) {
        ...
        case "checkbox":
            for (CommonCode commonCode : commonCodes) {
                if (StringUtils.isNotEmpty(defaultValue) && defaultValue.equals(commonCode.getCode())) {
                    builder.append(String.format("<label class=\"checkbox-inline\"><input type=\"checkbox\" name=\"%s\" value=\"%s\" checked> %s </label>", name, commonCode.getCode(), commonCode.getName()));
                } else {
                    builder.append(String.format("<label class=\"checkbox-inline\"><input type=\"checkbox\" name=\"%s\" value=\"%s\"> %s </label>", name, commonCode.getCode(), commonCode.getName()));
                }
            }
            break;
        ...
        case "radio":
            for (CommonCode commonCode : commonCodes) {
                if (StringUtils.isNotEmpty(defaultValue) && defaultValue.equals(commonCode.getCode())) {
                    builder.append(String.format(" <input type=\"radio\" name=\"%s\" value=\"%s\" checked> %s &nbsp;", name, commonCode.getCode(), commonCode.getName()));
                } else {
                    builder.append(String.format(" <input type=\"radio\" name=\"%s\" value=\"%s\"> %s &nbsp;", name, commonCode.getCode(), commonCode.getName()));
                }
            }
            break;
    }
